<html>
  ...
  <body>
    ...
    <div class="jumbotron">
      <div class="container">
        <h1><?php
          echo convertTo12Hour($_GET['time']);
        ?></h1>
      </div>
    </div>
    <div class="container">
        ...
        <tbody>
          <?php
            ...
            while($row = $result -> fetch_assoc()) {
              echo '<tr><td>' . $row['id'] . '</td><td>'. $row['date'] . '</td><td>' . parseTime($row['time_out'], true) . '</td><td>' . parseTime($row['time_in'], true) . '</td></tr>';
            }
          ?>
        </tbody>
      </table>
